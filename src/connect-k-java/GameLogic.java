...
public class GameLogic {
     ...
     */
    public GameLogic(int col, int row, int p,int g, String mode, int order) {
        this.col = col;
        this.row = row;
        this.p = p;
        this.g = g;
    ...
    private void Manual() throws InvalidParameterError {
        ...
        Board board = new Board(col, row, p,g);
        board.showBoard();
        while (true) {
            try {
    ...
    private void TournamentInterface() throws InvalidParameterError, InvalidMoveError {
        ...
        StudentAI ai = new StudentAI(col, row, p,g);
        while (true) {
    ...
    public void Run() throws InvalidParameterError, InvalidMoveError {
        if ("m".equals(this.mode) || "manual".equals(this.mode)) {
            if (this.order == 1) {
                aiList.addElement(new ManualAI(col, row, p,g));
                aiList.addElement(new StudentAI(col, row, p,g));
            }
            else {
                aiList.addElement(new StudentAI(col, row, p,g));
                aiList.addElement(new ManualAI(col, row, p,g));
